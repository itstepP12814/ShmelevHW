...
bool moveAll(std::string fromPath, std::string toPath, int choice){
	...
	if (doneTo == -1){
		...
		if (check_mkdir == -1){
			throw std::exception("Error: Can't create dirrectory for copying.\n");
		}
	}
	if (done != -1){
		int endFlag = done;
		...
		while (endFlag != -1){
			if (searchResultQueueFrom.attrib == _A_SUBDIR){ //Проверяем наличие поддиректории
				...
				if (dirRemoveCheck == -1){
					throw std::exception("Error: Can't remove a folder.\n");
				}
			}
		...
		if (doneTo == -1){
			int dirRemoveCheck = _rmdir(toPath.c_str());
			if (dirRemoveCheck == -1){
				printf("Error: Can't remove a folder.\n");
			}
		}
		throw std::exception("Error: Source path does not exist.\n");
	}
}
/*ѕри нажатии на кнопки 1 2 3 мен€ть изображение курсора*/
#include <windows.h>
#include "resource.h"
#include <tchar.h>
...
int WINAPI _tWinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPTSTR lpszCmdLine, int nCmdShow)
{
	...
	wcl.cbWndExtra = 0; 	
	wcl.hInstance = hInst;	
	wcl.hIcon = LoadIcon(hInst, MAKEINTRESOURCE(IDI_ICON1)); // иконка загружаетс€ из ресурсов приложени€	
	wcl.hCursor = LoadCursor(hInst, MAKEINTRESOURCE(IDC_CURSOR1));	// курсор загружаетс€ из ресурсов приложени€	
	...
	if (!RegisterClassEx(&wcl))
		return 0;
	hWnd = CreateWindowEx(0, szClassWindow, TEXT("–есурсы"), WS_OVERLAPPEDWINDOW,	
	...
	DWORD dwTime)     // current system time 
{
	...
	//ќпределение разрешени€ монитора
...
LRESULT CALLBACK WindowProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
		...
		case WM_CREATE:
			{
				...
				// загружаем иконку из ресурсов приложени€
				hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON2)); 
				// загружаем курсоры из ресурсов приложени€
		...
		case WM_KEYDOWN:{
							...
							if (wParam == '1'){
								SetCursor(hCursor1);
								SetClassLongPtr(hWnd, GCL_HCURSOR, LONG(hCursor1)); //«амен€ет хендл ресурса в окне
							}
							if (wParam == '2'){
								SetCursor(hCursor2);
							...
							if (wParam == VK_LEFT)
								SetTimer(hWnd, TIMER_LEFT, 1, TimerProc); //”станавливаем таймер дл€ перемещени€ окна
							if (wParam == VK_RIGHT)
